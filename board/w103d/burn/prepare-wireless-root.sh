#!/bin/bash
# Prepare v8 from an immutable, clean pairing-fixed v6 root.
set -euo pipefail
...
python3 "$scripts/desktop/verify-root.py" "$work/root" --before-permissions-fix > "$work/checks/baseline-desktop.json"
