...
# Rebuild the shared W103D/W102D bootstrap; retain the verified desktop payloads.
set -euo pipefail
if [ "$#" -lt 4 ] || [ "$#" -gt 5 ]; then
    echo "Usage: $0 BASE_ASSEMBLY NEW_WORK_DIR KHADAS_TOOLS LOGO_BMP [IMAGE_NAME]" >&2
    exit 2
fi
base=$(realpath "$1")
work=$(realpath -m "$2")
...
trap cleanup EXIT
mount -o loop,ro,noload "$base/rootfs.raw" "$checkroot"
...
cleanup
trap - EXIT
e2fsck -fn "$base/rootfs.raw" > /dev/null 2>&1
fsck.vfat -n "$base/bootfs.raw" > /dev/null 2>&1
test -f "$logo"
...
python3 "$shared/test_bootstrap.py" --output "$work/checks/bootstrap-tests"
...
name=${5:-W103D_Armbian_26.8.1_6.18.49_KDE_v8.burn.img}
[[ "$name" != */* && "$name" == *.burn.img ]] || exit 2
...
python3 "$shared/verify_capacity_rebuild.py" "$work" "$base"
