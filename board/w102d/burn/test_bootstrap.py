...
def fixture(path,sectors,corrupt=None):
    ept=bytearray(4096)
    ...
    if corrupt=='root-high':struct.pack_into('<I',ept,24+80+20,6)
...
def main():
    ...
    cases=[(n,None,None) for n in [28000000,30535680,30773248,31268864,31277056,32000000,33554432]]
    cases += [(31277056,c,None) for c in ['boot-offset','data-offset','root-high','root-low','ept','env','boot-label','root-label','root']]
    cases += [(31277056,'capacity',str(n)) for n in [0,16000000,27999999,33554433,60620800,67108864,4294967296]]
    cases += [(31277056,'capacity',s) for s in ['-1','invalid','31277056x']]
    results=[]
    with tempfile.TemporaryDirectory(dir=a.output) as tmp:
        ...
        for sectors,corrupt,argument in cases:
            ...
            if corrupt:
                ...
                assert b'6.18.49-w102d-16gb-v1\0' in env
    ...
    print(json.dumps(dict(passed=len(results),valid_capacities=7,rejected_cases=len(results)-7)))
