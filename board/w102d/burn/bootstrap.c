...
/* Runtime capacity, restricted to nominal 16 GB devices; never guess an LBA. */
#define MIN_EMMC_SECTORS 28000000u
#define MAX_EMMC_SECTORS 33554432u
...
static void stop(const char *s) {
    say("W102D 16GB bootstrap: ");say(s);say("\n");
#ifdef TEST_HOST
    sc(1,1,0,0,0,0);
#endif
    for(;;)sc(29,0,0,0,0,0);
...
static const char *changes[]={
    ...
    "upgrade_step=2", "w103d_burn_version=6.18.49-w102d-16gb-v1"
};
...
static void provision(int fd) {
    ...
    say("W102D 16GB bootstrap: boot entry installed and verified; starting Armbian\n");
}
...
void entry(unsigned *stack) {
    ...
    int fd=-1;
    /* Require eMMC boot0 and measured capacity in the nominal 16 GB class. */
    for(unsigned attempt=0;attempt<30 && fd<0;attempt++) {
        for(char i='0';i<='3';i++) {
            ...
            if(n<=0 || !capacity_ok(sectors))continue;
